...
      //  return this._http.get(this.weatherURL1 + city + this.weatherURL2+ state + this.weatherURL3);
    return this._http.get('https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20weather.forecast%20where%20woeid%20in%20(select%20woeid%20from%20geo.places(1)%20where%20text%3D%22nome%2C%20ak%22)&format=json&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys');

}
 
    findMovie(movie) {
        this.totReqsMade = this.totReqsMade + 1;
        //console.log( this._http.get(this.findMovieURL1 + movie + this.findMovieURL2 + this.key));
        return(this._http.get('http://www.omdbapi.com/?i=tt3896198&apikey=49f8cb2d'));
